...
# A vérifier ++
dev = 2*logLik(model) - 2*logLik(model_spline)
p = 1-pchisq(devm1indep ,df=1)
p
```
...
$$
\beta_{0X} = \mathbb{E}(\ln{Y_{ij}}|\alpha_{i}, X = 1) - \mathbb{E}(\ln{Y_{ij}}|\alpha_{i}, X = 0)
$$

et 

$$
e^{\beta_{0X}} = \frac{\mathbb{E}(Y_{ij}|\alpha_{i}, X = 1)}{\mathbb{E}(Y_{ij}|\alpha_{i}, X = 0)}
$$

Et représente donc la variation du score entraînée par la variable X, par rapport à la classe de référence, à l'entrée dans l'étude. Par exemple, les patients de Dijon ont, à l'entrée dans l'étude, un score CESDT $e^{0.43} = 1.54$ fois supérieur à ceux de Bordeaux.

De même, $e^{\beta_{1X}}$ représente la variation annuelle causée par la variable X, par rapport à la classe de référence. Par exemple, les patients de Dijon ont, un score CESDT qui décroit de $e^{-0.3}-1 = -0.03$ soit de 3% par an par rapport à ceux de Bordeaux.
...
Pour le modèle conjoint, il faut :
- un modèle mixte : `{r}model`
- un modèle de survie
...
```{r}
modelsurv = coxph(Surv(AGEDEM6, DEM0_6==1)~ SOM
                                          + I(SOM^2)
                                          + I(SOM^3)
                                          + SEXE
                                          + AGE0
