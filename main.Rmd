...
### Transformation de variable d'intérêt
...
On compare le modèle log au modèle SQRTCESDT :

```{r}
modelLogUni = lme(LOGCESDT ~ DELAI,
                  random = ~ DELAI | ID,
                  data=shatter,
                  method="ML",
                  na.action=na.omit)
...
```{r}
modelSqrtUni = lme(SQRTCESDT ~ DELAI,
                   random = ~ DELAI | ID,
                   data=shatter,
                   method="ML",
                   na.action=na.omit)
summary(modelSqrtUni)

AIC(modelLogUni)
AIC(modelSqrtUni)
```
On note que l'AIC du modèle log univarié est inférieur au modèle SQRT univarié.

### Modèle multivarié SOM
