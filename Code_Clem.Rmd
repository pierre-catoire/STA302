...
On regarde si ça change beaucoup si on enlève ou non la personne seule avec le score à 14:
```{r}
model_score_14 = lme(fixed = LOGCESDT ~ DELAI*(factor(CENTRE)
                                          + factor(ETUDE_CLAS0)
                                          + SEXE
                                          + ANTIDEP0
                                          + AGECENTER
                  ...
                  method="ML",
                  na.action=na.omit)
summary(model_score_14)
intervals(model_score_14,level=0.95,which="all")
```
...
devm0 <- 2*logLik(modelcentre) - 2*logLik(model_score_14)
p <- 1-pchisq(devm, df=3 )
p
#OU
devm0 <- 2*logLik(model_score_14) - 2*logLik(modelcentre)
p <- 1-pchisq(devm, df=3 )
p
...
```{r}
modeletude = lme(fixed = LOGCESDT ~ DELAI*(factor(CENTRE)
                                          + SEXE
                                          + ANTIDEP0
                                          + AGECENTER
                                          + SCORE0
...
devm1 <- 2*logLik(modeletude) - 2*logLik(model_score_14)
p <- 1-pchisq(devm, df=2 )
p
#OU
devm1 <- 2*logLik(model_score_14) - 2*logLik(modeletude)
p <- 1-pchisq(devm, df=2 )
p
```
